...
app.use(express.urlencoded()); // recognizes request object as strings or arrays
...
//---------------------------------- Standard Routes ----------------------------------------------------//
app.use((req, res) => res.status(404).send('This is not the page you\re looking for.')) // catch all error handler


app.use((err, req, res, _next) => { // global error handler 
    const defaultError = {
