...
perksController.addPerk = async (req, res, next) => {
  const { perk_name, tokens, qty, perk_img } = req.body;

  if (!perk_name || !tokens || !qty)
    return res.status(400).json({ error: 'Request is missing a parameter' });

  const query = `
  INSERT INTO perks (perk_name, tokens, qty_initial, qty_remaining, id, perk_img)
  VALUES ($1, $2, $3, $3, (SELECT COALESCE(MAX(id), 0) + 1 FROM perks), $4)
  RETURNING *;
`;
...
perksController.buyPerk = async (req, res, next) => {
  ...
  if (user_tokens < perk_tokens)
    return res.status(400).json({ error: 'User does not have enough tokens' });
  ...
  try {
    const decrementUserTokens = `
        ...
        RETURNING *;
        `;
    const userTokens = await pool.query(decrementUserTokens, [
      perk_tokens,
      user_id,
    ]);
